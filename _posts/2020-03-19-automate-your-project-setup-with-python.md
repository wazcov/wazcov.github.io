...
<pre class="wp-block-code"><code class="">source ~/.mycommands.sh</code></pre>

Then create a file in your home directory ~/.mycommands.sh with this inside:

<pre class="wp-block-code"><code class="">function build() {
	./buildProject.py $1
...
<pre class="wp-block-code"><code class="">#! /usr/bin/python3
import sys
import os
import time
...
g = Github("XXX")
user = g.get_user()
repo = user.create_repo(project_name)</code></pre>
